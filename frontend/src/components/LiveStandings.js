...
const LiveStandings = ({ positions, onSelectHorse }) => {
  ...
  // First finisher reference for post-race gap calculation
  const firstFinishTime = positions?.[0]?.finish_time ?? null;
  const leaderDistance = positions?.[0]?.distance ?? 0;

  if (!positions || positions.length === 0) {
    return <div className="text-gray-500 text-center py-4">Waiting for race data...</div>;
  }

  // Convert time gap to racing margin relative to winner finish time
  const getMarginFromFinishTime = (currentHorse) => {
    // During race: show distance to current leader
    if (!firstFinishTime || !currentHorse.finish_time) {
      const gap = leaderDistance - currentHorse.distance;
    ...
    const timeDiff = currentHorse.finish_time - firstFinishTime; // slower horse has larger time
